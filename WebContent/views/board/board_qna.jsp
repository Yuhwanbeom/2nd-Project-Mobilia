...
<form method="get" action="qna.net">
	<div class="title-area">
		<h2>Q&A</h2>
		<label class="info-text">Q&A 게시글을 볼수있는 공간입니다.</label> <input
 ...
 <c:forEach var="q" items="${qlist}">
	<div class="write-list-area">
		<div id="board-title-area">
			<label><a href="#">${q.board_no}. ${q.board_title} (${q.reply_hit})</a></label><br>
		</div>
   ...
   <div id="bList_paging">
    ...
    <c:if test="${(empty find_field) && (empty find_name)}"> <%--검색필드와 검색어가 없는 경우 --%>
     ...
     <c:if test="${page>1}">
      <a href="qna.net?page=${page-1}">[이전]</a>&nbsp;
     </c:if>
    ...
    <c:if test="${page < maxpage}">
     <a href="qna.net?page=${page+1}">[다음]</a>
    </c:if>
   </c:if>
    ...
    <c:if test="${(!empty find_field) && (!empty find_name)}"> <%--검색필드와 검색어가 있는 경우 --%>
     ...
     <c:if test="${page>1}">
      <a href="qna.net?page=${page-1}&find_field=${find_field}&find_name=${find_name}">[이전]</a>&nbsp;
    ...
    <c:if test="${page < maxpage}">
     <a href="qna.net?page=${page+1}&find_field=${find_field}&find_name=${find_name}">[다음]</a>
    </c:if>
   </c:if> 
   </div>
   </form>
